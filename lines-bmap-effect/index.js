...
module.exports = Event.extend(function Base(container, config) {
  ...
  init: function (config) {
    ...
    script.src = "http://api.map.baidu.com/api?v=2.0&ak=VVIT2zwXigdR3oK2nTk5iP4y9MGVT9xa&callback=initialize";
  ...
  render: function (data, config) {
    ...
    var formatData = data.map(function (item, idx) {
      return {
        coords: item.coords,
        lineStyle: {
          normal: {
            color: Chart.color.modifyHSL('#5A94DF', Math.round(hStep * idx))
          }
        }
      }
    });
